...
const Game = ({data}) => {
    ...
    const handleClick = (e) => {
        ...
        if(newValues.length === 2){
            if(data[newValues[0]] === newValues[1] ||  data[newValues[1]] === newValues[0]){
                ...
                gameData.length && (
                    <ul>
                        {gameData.map((option) => {
                            ...
                            const isCorrect = selectedOptions.length===2 && correctAns.includes(option)
                            const isInCorrect = selectedOptions.length===2 && inCorrectAns.includes(option)
                            return (
                                <li className={`${isSelected && 'selected'} ${isCorrect && 'correct'} ${isInCorrect && 'inCorrect'}`} key={option} onClick={handleClick} data-value={option}>
                                    {option}
                                </li>
                            )
                        })}
